...
# This creates a data frame of all continuous variables.
...
creditDf %>%
  ...
  ylab('Count') +
  xlab('Default (1 = yes)') +
  ggtitle('Figure1: The Number of Defaults in the Dataset') +
  theme_minimal()
...
creditDf %>%
  ...
  ggtitle('Figure 2: The Proportion of Default by Education, Sex, and Marriage') +
  theme_minimal() +
  theme(
...
creditDf %>%
  ...
  xlab('Age') +
  ylab('Proportion') +
  ggtitle('Figure 3: The Proportion of Default by Age and Sex') +
  labs(fill = 'Default 1 = yes | 0 = no') +
  theme_minimal() +
  theme(
    legend.position = 'bottom'
...
creditDf %>%
  ...
  ggtitle('Figure 4: Density Plots of Bill Amounts') +
  labs(fill = 'Default 1 = yes | 0 = no') +
  theme_minimal() +
  theme(
    legend.position = 'bottom',
...
creditDf %>%
  ...
  labs(fill = 'Default 1 = yes | 0 = no') +
  ggtitle('Figure 5: Density Plits of Pay Amounts') +
  theme_minimal() +
  theme(
    legend.position = 'bottom',
    axis.text.x = element_text(angle = 45, hjust = 1)
...
creditDf %>% 
  ...
  ggtitle('Figure 6: Boxplots of the Variables') +
  theme_minimal() +
  theme(
...
categorical %>%
  ...
  xlab('Variable') +
  ylab('Count') +
  ggtitle('Figure 7: Histograms of All Categorical Variables') +
  theme_minimal() +
  theme(
    #axis.text.x = element_text(angle = 45, hjust = 1),
    axis.text.y = element_blank(),
...
continuous %>%
  ...
  facet_wrap(~var, scales = 'free') +
  ylab('Density') +
  ggtitle('Figure 8: Density Plots of All Continuous Variables') +
  theme_minimal() +
  theme(
    #axis.text.x = element_text(angle = 45, hjust = 1),
...
lbdist = ggplot(creditDf_LB, aes(x=factor(LIMIT_BAL), y=Counts, fill=factor(default))) + 
      ...
      ylab('Total')+
      theme_minimal()+
      ggtitle("Figure 13: Limit Balance Distribution") +
      theme(
        axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = 'bottom'
      )
...
genderdist = ggplot(creditDf_Sex, aes(x=factor(SEX), y=Count, fill=factor(default))) + 
      ...
      ylab('Total')+
      theme_minimal()+
      ggtitle("Figure 14: Gender Distribution") +
      theme(
        legend.position = 'bottom'
      )
...
edudist = ggplot(creditDf_Edu, aes(x=factor(EDU), y=Count, fill=factor(default))) + 
      ...
      ylab('Total')+
      theme_minimal()+
      ggtitle("Figure 15: Educatiton Distribution")
...
mardist = ggplot(creditDf_Mar, aes(x=factor(MAR), y=Count, fill=factor(default))) + 
      ...
      ylab('Total')+
      theme_minimal()+
      ggtitle("Figure 16: Marriage Distribution") +
      theme(
        legend.position = 'bottom'
      )
mardist = mardist + guides(fill=guide_legend(title ='Default Payment: 0 = No Default | 1 = Default' )) ; mardist
##Married and Single Customers have a approx the same default rate.
...
agedist = ggplot(creditDf_Age, aes(x=age, y=Count, fill=factor(default))) + 
      ...
      ylab('Total')+
      theme_minimal()+
      ggtitle("Figure 17: Age Distribution") +
      theme(
        legend.position = 'bottom'
      )
...
logit.comparison %>%
  ...
  geom_col() +
  ggtitle('Figure 18: AIC for Each Logistic Model') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
  )
...
plot(
  ...
  'tpr',
  'fpr'
  ), colorsize = TRUE, main = 'Figure 20: ROC Curve of Logistic Models'
  )
...
# The accuracy is stored in a variable for later use. 
...
plot(perf, main = 'Figure 22: ROC Curve of Random Forest Classifer')
...
model.comparison %>%
  ...
  geom_col() +
  ggtitle('Figure 23: Accuracy of the Models') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
  )
...
In this section we reviewed the effectiveness of utilizing demographic variables (collected prior to credit card issument) to determine if they are a viable method for predicting default payment among our clients. In short - they are not. While the demographic variables  are significant predictors of default. However, our model that solely relies on demographic variables to make predicts has a low accuracy. Meaning, that our demographic variables as a base are not helpful in determining who might default. With this in mind, we’ll pivot here to focus on all known variables to build our model (either existing, engineered, or inputted in to our machine learning systems) 

We inputted our data into three (3) machine learning classification models:
* K-Nearest Neighbors
* Random Forest
* Gradient Boosting

In terms of accuracy, the K-Nearest Neighbors offer the highest likelihood of predicting default payment by utilizing all of our continuous variables. To note, the only continuous Demographic variable included would be age.  

Recommendations:
* When issuing credits, ignore demographic data as it is  poor method for predicting default payment 
* Our strongest predictors of default tend to fall within the payment category, specifically ‘Pay_0’ and ‘Pay_Avg.’
* With this in mind, we would recommend that your Bank should focus its efforts on early payment reminders,
* In addition to early reminders, we would also push for regularly on-time payment incentives - ranging from local, gift cards to discounted services. 
* Implement optional spending controls after Pay_6 for customers that want to manage their debt and climb back to Pay_0 status. 

...
# This will create new training and testing data sets. However, the continupus variables will be scaled. 

train_scaled <- train %>%
  select(c(limit_bal, age, bill_amt1, bill_amt2, bill_amt3, bill_amt4, bill_amt5, bill_amt6, 
           pay_amt1, pay_amt2, pay_amt3, pay_amt4, pay_amt5, pay_amt6,
           bill_avg, pay_avg, avg_credit_usage)) %>%
  scale() %>%
  data.frame()

train_scaled <- cbind(train$default.payment.next.month, train_scaled)
...
test_scaled <- cbind(test$default.payment.next.month, test_scaled)

test_scaled <- test_scaled %>%
  rename(
      default.payment.next.month = `test$default.payment.next.month`
  )
...
logit_model3B <- glm(default.payment.next.month ~ limit_bal + age + 
                    pay_amt1 + avg_credit_usage,
                    data = train_scaled, family = 'binomial')
